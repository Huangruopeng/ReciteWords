...
void Widget::initUi()
{
    ...
    //设置表头
    QStringList header;
    header << tr("用户名") << tr("IP") << tr("端口") << tr("已背单词个数") << tr("正确率(%)") << tr("使用时间(s)");
...
void Widget::updateUserData(QString *data)
{
    ...
    if(row > 0){
      int i = 0;
      for(i = 0;i < row; ++i){
            QString strText = ui->tableWidget->item(i,2)->text();//端口号
    ...
    ui->tableWidget->sortItems( 4, Qt::DescendingOrder);//按正确率排
